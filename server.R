...
server <- function(input, output, session) {
  ...
  output.table <- reactive({
    ...
    else if (input$table_picker =="Grouped") {
        d <- process.data()
    }
    else {
  ...
  plot_bar <- function(var) {
    ...
    p <- ggplot(data = d, aes(x = group, y = mean, fill = as.factor(group))) +
      ...
      scale_x_discrete(name = "", limits = get.treat.names())
  ...
  # helper for plotSwarm
  plot_swarm <- function(var) {
    d <- process.data()
    
    # Ensure the data is in the correct long format
    d <- d %>%
      mutate(across(everything(), as.character)) %>%
      pivot_longer(cols = -group, names_to = "variable", values_to = "value")
    
    p <- d %>%
      filter(variable == var) %>%  # Filter for the selected variable
      ggplot(aes(x = as.factor(group), y = as.numeric(value), fill = as.factor(group))) +
      geom_boxplot(fill = "white", color = "black", outlier.shape = NA) +
      geom_jitter(width = 0.15, size = 1.5, aes(color = as.factor(group))) +
      labs(y = clean_name(var), x = "", fill = "Treatment", color = "Treatment") +  # Explicitly set legend titles
      theme(
      ...
      scale_x_discrete(name = "", labels = get.treat.names()) +  # Use custom labels for x-axis
      scale_fill_manual(values = allColors(), labels = get.treat.names()) + # Custom colors for boxplot
      scale_color_manual(values = allColors(), labels = get.treat.names())   # Custom colors for dots
